...
/**
    ...
    *
    * Функция удаляет пустые элементы посредством метода array_diff() и пустые строки посредством метода unset(), осуществляет проверку размера матрицы, наличия элементов, не сооветствующих формату входных данных, и равенства элементов на главной диагонали единице
    *
...
function validate(& $matrix)
{
    ...
    for ($i = 0; $i < count($matrix); $i++)
    {
        if ($matrix[$i][$i] == 0)
        {
            echo "Элемент, находящийся на главной диагонали, равен нулю!";
            exit;
        }
    }
}
